...
function BudgetScheduleCircle2({ setCategoryPositions }) {
    const [categories, setCategories] = useState([
        { id: 1, name: '약속', color: '#5E64FF', budget: 150 },
        { id: 2, name: '시험', color: '#A045E7', budget: 20 },
        { id: 3, name: '휴가', color: '#8F64A9', budget: 100 },
        { id: 4, name: '문화생활', color: '#ff8585', budget: 80 },
        { id: 5, name: '출장', color: '#A86EAD', budget: 20},
        ...
        if (!isNaN(newRatio) && newRatio >= 0 && newRatio <= 100) {
            ...
            if (newTotalSaved >= 0) {
                ...
                d3.select(svgRef.current)
                    ...
                    .transition()
                    .duration(500)
                    .attr('fill', d => d3.color(d.color).opacity(0.3));
            } else {
    ...
    const drawChart = () => {
        ...
        const nodeGroup = svg.selectAll('g')
            ...
            .enter()
            .append('g')
            .attr('transform', d => `translate(${d.x + 20}, ${d.y - 60})`)
        ...
        nodeGroup.each(function (d) {
            ...
            // d.data.budget의 60%와 40% 대신 랜덤한 비율을 사용하여 파이 데이터 생성
            const randomValue1 = 1.4 + Math.random() * (1.4 - 1.1); // 0.4와 1 사이의 랜덤 값
            const randomValue2 = 1.1 - randomValue1; // 두 값의 합이 1이 되도록 설정

            const pieData = pie([
                { value: d.data.budget * randomValue1, type: 'actual' },
                { value: d.data.budget * randomValue2, type: 'remaining' }
            ]);
            ...
            const paths = g.selectAll('path')
                ...
                .attr('opacity', pieSlice => pieSlice.data.type === 'actual' ? 0.8 : 0.3)
                .attr('d', arc);
    ...
    return (
        <div className="fluid-account-bubble-lq-c">
            <div className="_6-lq-c" style={{ top: '90px' }}>{/*currentMonth*/}5월 변동지출</div>
            <div className="total-budget-lq-c">총 {formatBudget(totalBudget)}</div>
            <div className="_38-lq-c">{monthlyBudget}</div>
            <div className="saving-lq-c-over">초과지출: 3만원</div>
            <div className="line-3-lq-c-over"></div>
            {isModifying ? (
                <button 
            ...
            <svg ref={svgRef} className="vector-lq-c" onClick={navigateToPresent} width="6" height="12" viewBox="0 0 6 12" fill="none" xmlns="http://www.w3.org/2000/svg">
...
export default BudgetScheduleCircle2;
